...
export function AppLayout() {
  ...
  return (
    <div className="min-h-dvh overflow-x-hidden text-slate-900 transition-colors dark:text-slate-100">
      <div
        className={cn(
          'mx-auto flex min-h-dvh w-full max-w-[1440px] gap-2 px-2 pt-2 sm:gap-4 sm:px-4 sm:pt-4 lg:gap-6 lg:px-6 xl:pb-4',
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <AppTopbar />
          <main className="mt-2 flex-1 overflow-x-hidden p-3 animate-fade-in-up transition-colors sm:mt-4 sm:rounded-3xl sm:border sm:border-slate-200/90 sm:bg-white/95 sm:p-5 sm:shadow-panel lg:p-6 dark:sm:border-slate-700/90 dark:sm:bg-slate-900/90">
            <Outlet />
          </main>
        </div>
      </div>
